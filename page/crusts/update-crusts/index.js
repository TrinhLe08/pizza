...
const UpdateCrustsPage = () => {
  const valueToUpdate = useRecoilValue(recoilCrust.informationToUpdateCrust)
  ...
  const [description_v, setDescription_v] = useState(valueToUpdate.description_v);
  ...
  const handleChangeUpdate = async () => {
    const dataToUpdate = { code, name, name_v, description, description_v, order_by, active: true };
    console.log(dataToUpdate);
    try {
      const update = await CrustsApi.updateCrusts(dataToUpdate, valueToUpdate._id)
      console.log(update);
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={styles.label}>Description Vi:</Text>
      <TextInput
        style={styles.input}
        value={description_v}
